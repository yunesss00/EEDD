...
int compute_height (typename BTree<T>::Ref t)
{
    ...
    /*else if (t!=is_empty() && t->has_left() == false && t->has_right() == false) //t no childs
    {
        aux++;
        if (ret_val < aux) ret_val = aux;
    ...
    {
        if (t->rootptr->has_left() == true)
        {
            aux++;
            if (ret_val < aux) ret_val = aux;
            compute_height(rootptr->left());
        }

        if (t->rootptr->has_right() == true)
        {
            aux++;
            if (ret_val < aux) ret_val = aux;
            compute_height(rootptr->right());
        }
